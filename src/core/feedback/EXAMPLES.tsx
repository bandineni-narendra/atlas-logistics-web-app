...
export function Example4_Warning() {
  ...
  const handleDelete = () => {
    openWarningModal("This action cannot be undone. Are you sure?", () => {
...
export function Example5_Validation() {
  const { state, openValidationModal, closeValidationModal } =
    useFeedbackModal();
...
export function createCustomValidationAdapter() {
  return (data: CustomData[], sheetName: string): ValidationResult => {
    ...
    data.forEach((item, index) => {
      ...
      if (
        issues.filter((i) => i.severity === "error" && i.rowIndex === rowIndex)
          .length === 0
      ) {
        validCount++;
      }
