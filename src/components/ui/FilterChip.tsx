...
export const FilterChip: React.FC<FilterChipProps> = ({
    ...
    return (
        ...
        ${isActive
                    ...
                    : "bg-transparent text-textSecondary border-border hover:bg-surface"
                }
        ${className}
      `}
        >
            ...
            {icon && !isActive && <span className="text-primary">{icon}</span>}

            <span>{label}</span>

            {/* Trailing Icon (Dropdown arrow or Close 'X') */}
            {trailingIcon && (
                <span className={`ml-1 ${isActive ? "text-primary" : "text-textSecondary"}`}>
                    {trailingIcon}
                </span>
            )}
        </button>
