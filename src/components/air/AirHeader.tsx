"use client";

import { useTranslations } from "next-intl";
import { Card, CardContent, ConfidenceIndicator } from "@/components/ui";

export type AirHeaderProps = {
  confidence: number;
  fileName?: string;
};

/**
 * Air freight results header
 * Displays confidence score in a metric card
 */
export function AirHeader({ confidence, fileName }: AirHeaderProps) {
  const t = useTranslations();

  return (
    <Card padding="md">
      <CardContent>
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-medium text-textSecondary">
              {t("air.confidence")}
            </h3>
            <div className="mt-2">
              <ConfidenceIndicator value={confidence} size="md" />
            </div>
          </div>
          {fileName && (
            <div className="text-right">
              <p className="text-sm text-textSecondary">{t("common.file")}</p>
              <p className="text-sm font-medium text-textPrimary mt-1">
                {fileName}
              </p>
            </div>
          )}
        </div>
      </CardContent>
    </Card>
  );
}
